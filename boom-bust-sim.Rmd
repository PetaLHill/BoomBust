...
The code below allows you to rerun the simulations as described in the Manuscript of Hill et al. (2023): Fragmentation and gene flow in a boom-bust metapopulation reveal a trade-off between heterozygosity and allelic richness.

As the actual simulation and analysis takes some time to run (about 2 days using 40 cores) we also provide means to run the simulation with fewer parameter combinations. 
...
For the R installation you need to have a recent version of R (>4.0) and the following packages. Please note the version is important as some of the packages are known not to work if versions are different.
...
slim_script(
  slim_block(initialize(),
             { initializeSLiMModelType("nonWF");
               defineConstant("alpha1", 1e-9); #mutation rate
               defineConstant("metasz",256);# total number of individuals in the metapopulation
               defineConstant("subp_num",slimr_template("pNum"));#number of subpopulations
               defineConstant("rep",slimr_template("rep")); #number of replicate simulations
               defineConstant("mig", slimr_template("mig")); #migration rate as percent
               defineConstant("migRate",asInteger(mig/100*metasz)); #migration rate as number of individuals
               defineConstant("subp_sz", asInteger(metasz/subp_num)); #number of individuals in each subpopulation
               defineConstant("outpath", slimr_inline(outdir)); #output directory
               defineConstant("mev",slimr_template("mev"));# How often to mix
               initializeSLiMOptions(nucleotideBased=T);
               defineConstant("L",slimr_inline(n.loc)) 
               initializeAncestralNucleotides(slimr_inline(R)); #reference sequence
  ...
  slim_block(1, {
    for (i in seqLen(subp_num)){#set up subpopulations, each of size 'subpop_sz', reading in genotypes
  ...
  slim_block(2,499, first(), {
    if(sim.generation %% mev == 0){#if the generation is one in which gene flow is to occur, take migrants into the migrant pool
      migrants = sample(sim.subpopulations.individuals,migRate);
      p100.takeMigrants(migrants);
    }
  }),
  slim_block(2,500, first(), {
    if(sim.generation %% mev == 1){#if the generation is the one after migration, randomly return migrants to the subpopulations
...
script_temp_df <- slim_script_render(BBSc3, template =df)#generates a simulation for all combinations of parameters 
...
ext <- function(i) {
  dumvcf<- read.vcfR(file.path(outdir,filesM[i]),verbose=FALSE)
